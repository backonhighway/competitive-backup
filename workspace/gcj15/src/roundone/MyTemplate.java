...
public class MyTemplate implements Runnable {
	...
	public MyTemplate(int caseNum, Scanner in)
	{
	...
	public static void main(String[] args) throws Exception
	{
		...
		CompletionService<MyTemplate> cs = new ExecutorCompletionService<MyTemplate>(es);
		...
		preprocess();
		for(int i = 0;i < n;i++){
			MyTemplate runner = new MyTemplate(i+1, in);
			runner.read();
		...
		for(int i = 0;i < n;i++){
			MyTemplate runner = cs.take().get(); // not ordered
			runner.out.flush();
			runner.out.close();
