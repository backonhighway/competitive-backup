...
public class TaroFriends
{
	...
	public int getNumber(int[] coordinates, int X)
	{
		...
		for (int i = 0; i < len; i++) {
			pos[i*2] = coordinates[i] - X;
			pos[i*2+1] = coordinates[i] + X;
		}
		
		int ret = INF;
		for (int i = 0; i < pos.length; i++) {
			for (int j = 0; j < pos.length; j++) {
				int posleft = Math.min(pos[i], pos[j]);
				int posright = Math.max(pos[i], pos[j]);
				boolean ok = true;
				for (int k = 0; k < len; k++) {
					int left = coordinates[k] - X;
					int right = coordinates[k] + X;
					boolean satisfy = (posleft <= left && left <= posright)
							|| (posleft <= right && right <= posright);
					if (!satisfy) {
						ok = false;
					}
				}
				if (ok) ret = Math.min(ret, posright - posleft);
			}
		}
		return ret;
	}
