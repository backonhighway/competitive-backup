...
public class Subsets
{
	...
	public int findSubset(int[] numbers)
	{
		...
		int ret = dfs(oneCount, 1, 0);
		return ret;
	}
	
	public int dfs(int index, int product, int sum) {
		if (index == len || product*numbers[index] >= sum+numbers[index] + oneCount) {
			return Math.max(0, oneCount + sum - product);
		}
		
		int ret = 0;
		//pick index
		ret += dfs(index+1, product*numbers[index], sum+numbers[index]);
		//dont pick index
		ret += dfs(nextIndex[index], product, sum);
		return ret;
	}
