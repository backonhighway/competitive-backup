...
// BEGIN CUT HERE
   public static void main(String[] args) {
		if (args.length == 0) {
			FallingFactorialPowerHarness.run_test(-1);
		} else {
			for (int i=0; i<args.length; ++i)
				FallingFactorialPowerHarness.run_test(Integer.valueOf(args[i]));
		}
	}
// END CUT HERE
}
...
// BEGIN CUT HERE
class FallingFactorialPowerHarness {
	...
	static int runTestCase(int casenum__) {
		...
		case 0: {
			int n                     = 7;
			int k                     = 3;
			double expected__         = 210.0;

			return verifyCase(casenum__, expected__, new FallingFactorialPower().compute(n, k));
		}
		case 1: {
			int n                     = 10;
			int k                     = 1;
		...
		case 4: {
			int n                     = 2;
			int k                     = -3;
			double expected__         = 0.016666666666666666;
